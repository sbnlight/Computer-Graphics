...
bool Sphere::Hit(const Ray &ray, HitRecord *hit_record) const {
    ...
    float epsilon = 1e-4f; // Small epsilon to prevent self-intersection

    // If the first root is too close (or behind), try the second root
    if (t < epsilon) {
        t = (-b + sqrt_d) / (2.0f * a);
    }
...
bool Triangle::Hit(const Ray &ray, HitRecord *hit_record) const {
    ...
    float a_param = glm::dot(edge1, h);

    // If a_param is near zero, the ray lies in the plane of the triangle
    if (a_param > -epsilon && a_param < epsilon)
        return false;

    float f = 1.0f / a_param;
    Vec s = ray.o - a_;
    ...
    {
        hit_record->distance = t;
        hit_record->position = ray.o + ray.d * t;
        hit_record->in_direction = ray.d;
        ...
        if (phong_interpolation_) {
            ...
            // w = 1 - u - v
            float w = 1.0f - u - v;
            hit_record->normal = glm::normalize(w * n_a_ + u * n_b_ + v * n_c_);
        } else {
            // Use face normal (computed from edges or pre-stored n_a_ if flat)
            // Usually for flat shading, the face normal is constant. 
            // We can calculate it dynamically or use n_a_ assuming consistent data.
            hit_record->normal = glm::normalize(glm::cross(edge1, edge2));
        }

        hit_record->reflection = ray.d - 2.0f * glm::dot(ray.d, hit_record->normal) * hit_record->normal;
        
        // Note: material assignment is handled in CompleteTriangle wrapper, 
        // but we can ensure internal state is clean.
        return true;
    }
